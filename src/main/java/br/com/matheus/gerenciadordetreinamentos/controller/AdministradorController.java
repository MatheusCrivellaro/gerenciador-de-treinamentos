...
public class AdministradorController {
    ...
    @ApiResponses(value = {
            @ApiResponse(description = "Sucess", responseCode = "200", content = @Content(schema = @Schema(implementation = AdministradorViewDTO.class))),
    ...
    @GetMapping("/{id}")
    public ResponseEntity<AdministradorViewDTO> findById(@PathVariable Long id) {
    ...
    @ApiResponses(value = {
            @ApiResponse(description = "Sucess", responseCode = "200", content = {
                    @Content(mediaType = "application/json", array = @ArraySchema(schema = @Schema(implementation = AdministradorViewDTO.class)))
            }),
            @ApiResponse(description = "Bad Request", responseCode = "400", content = @Content),
            @ApiResponse(description = "Unauthorized", responseCode = "401", content = @Content),
            @ApiResponse(description = "Not Found", responseCode = "404", content = @Content),
    ...
    @GetMapping
    public ResponseEntity<List<AdministradorViewDTO>> findAll() {
        return ResponseEntity.ok(service.findAll().stream().map(AdministradorDTO::toView).toList());
    }
    ...
    @GetMapping("/nome")
    public ResponseEntity<List<AdministradorViewDTO>> findByNome(@RequestParam String nome) {
        return ResponseEntity.ok(service.findByNome(nome).stream().map(AdministradorDTO::toView).toList());
    }
    ...
    @GetMapping("/usuario")
    public ResponseEntity<List<AdministradorViewDTO>> findByUsuario(@RequestParam String usuario) {
        return ResponseEntity.ok(service.findByUsuario(usuario).stream().map(AdministradorDTO::toView).toList());
    }
    ...
    @PostMapping
    public ResponseEntity<AdministradorViewDTO> save(@Valid @RequestBody AdministradorSaveDTO data) {
    ...
    @PutMapping
    public ResponseEntity<AdministradorViewDTO> update(@Valid @RequestBody AdministradorUpdateDTO data) {
